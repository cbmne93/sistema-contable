...
export async function GET(_request: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        const txtFileName = getMarangatuFileName({
        ...
        const content = `${registro.detalles
            .map((detalle) => detalle.lineaTxt)
            .join("\r\n")}\r\n`;

        const zip = new JSZip();

        zip.file(txtFileName, content);
        ...
        console.error("Error al descargar ZIP TXT Marangatu:", error);

        return new Response("No se pudo descargar el archivo ZIP TXT.", {
            status: 500,
        });
    }
}