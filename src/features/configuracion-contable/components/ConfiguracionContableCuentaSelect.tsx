...
import type { ConfiguracionContableCuentaOption } from "../interfaces";

interface ConfiguracionContableCuentaSelectProps {
    label: string;
    description?: string;
    value?: string | null;
...
export function ConfiguracionContableCuentaSelect({
    label,
    description,
    value,
    cuentas,
    error,
    onChange,
}: ConfiguracionContableCuentaSelectProps) {
    ...
    return (
        <div className="space-y-1.5">
            ...
            <div
                ...
                className={
                    error
                        ? "flex h-10 w-full cursor-pointer items-center justify-between gap-2 rounded-lg border border-red-300 bg-white px-3 text-left text-sm text-slate-900 outline-none transition focus:border-red-400 focus:ring-2 focus:ring-red-100"
                        : "flex h-10 w-full cursor-pointer items-center justify-between gap-2 rounded-lg border border-slate-300 bg-white px-3 text-left text-sm text-slate-900 outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200"
                }
            >
                <span className="min-w-0 truncate">
                    {selectedCuenta
