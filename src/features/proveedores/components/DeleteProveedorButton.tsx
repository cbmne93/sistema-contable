...
export function DeleteProveedorButton({
    proveedorId,
    proveedorNombre,
    tieneMovimientos = false,
    estado = "ACTIVO",
}: DeleteProveedorButtonProps) {
    ...
    const handleAction = () => {
        ...
        startTransition(async () => {
            const response = await deleteProveedorAction({ id: proveedorId });
            ...
            <AlertDialogContent className="max-w-md">
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="space-y-3 text-sm leading-6">
                        ...
                        {tieneMovimientos ? (
                            <span className="block">
                                Este proveedor ya tiene comprobantes o timbrados
                                asociados, por eso no se eliminará de la base de
                                datos. Solo quedará marcado como inactivo.
                            </span>
                        ) : (
                            <span className="block">
                                Este proveedor no tiene movimientos asociados,
