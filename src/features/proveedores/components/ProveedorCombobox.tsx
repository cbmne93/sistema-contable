...
interface ProveedorComboboxOption {
    id: string;
    nombre: string;
...
interface ProveedorComboboxProps {
    value: string;
    proveedores: ProveedorComboboxOption[];
    error?: string;
    disabled?: boolean;
...
function getProveedorDocumento(proveedor: ProveedorComboboxOption) {
...
function getProveedorLabel(proveedor: ProveedorComboboxOption) {
    const documento = getProveedorDocumento(proveedor);

    if (!documento) {
        return proveedor.nombre;
    }

    return `${proveedor.nombre} - ${documento}`;
}
...
export function ProveedorCombobox({
    ...
    const filteredProveedores = useMemo(() => {
        ...
        if (!searchValue) {
            return proveedores.slice(0, 50);
        }

        return proveedores
            .filter((proveedor) => {
                const nombre = normalizeText(proveedor.nombre);
                const documento = normalizeText(
                    getProveedorDocumento(proveedor)
                );
                const textoCompleto = normalizeText(
                    `${proveedor.nombre} ${getProveedorDocumento(proveedor)}`
                );
    ...
    const handleSelect = (proveedorId: string) => {
                        ...
                        <input
                            ...
                            onKeyDown={(event) => {
                                ...
                                if (event.key === "Enter") {
                                    ...
                                    const proveedor =
                                        filteredProveedores[activeIndex];

                                    if (proveedor) {
                                        handleSelect(proveedor.id);
                                    }
                                }
                            }}
                    ...
                    <div className="max-h-72 overflow-y-auto py-1">
                        {filteredProveedores.length === 0 ? (
                            ...
                            filteredProveedores.map((proveedor, index) => {
                                const isSelected = proveedor.id === value;
            ...
            <div className="relative">
                ...
                >
                    <span className="min-w-0 truncate">
                        {selectedProveedor
                            ? getProveedorLabel(selectedProveedor)
