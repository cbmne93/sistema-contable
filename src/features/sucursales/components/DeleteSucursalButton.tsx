...
import { deleteSucursalAction } from "../actions";
...
export function DeleteSucursalButton({
    sucursalId,
    sucursalNombre,
    tieneMovimientos = false,
    estado = "ACTIVO",
}: DeleteSucursalButtonProps) {
    const router = useRouter();
    const [open, setOpen] = useState(false);
    const [isPending, startTransition] = useTransition();

    const estaInactiva = estado === "INACTIVO";
    const accion = tieneMovimientos ? "desactivar" : "eliminar";
    const titulo = tieneMovimientos ? "Desactivar sucursal" : "Eliminar sucursal";
    ...
    const handleAction = () => {
        if (isPending || estaInactiva) {
            return;
        }

        startTransition(async () => {
            const response = await deleteSucursalAction({ id: sucursalId });
                ...
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="space-y-3 text-sm leading-6">
                        <span className="block">
                            ¿Está seguro que desea {accion} la sucursal{" "}
                        ...
                        {tieneMovimientos ? (
                            <span className="block">
                                Esta sucursal ya tiene movimientos asociados, por
                                eso no se eliminará de la base de datos. Solo
                                quedará marcada como inactiva.
                            </span>
                        ) : (
                            <span className="block">
                                Esta sucursal no tiene movimientos asociados, por
